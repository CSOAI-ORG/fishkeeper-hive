...
```
L7  PRESENTATION    aquarium teal + coral pink (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      fishkeeper-ai-mcp, pet-care-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: freshwater/saltwater species, water chemistry, cycling, diseases, compatibility
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "fishkeeper"
...
```bash
git clone https://github.com/CSOAI-ORG/fishkeeper-hive
cd fishkeeper-hive
...
`https://fishkeeper.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Consumer subscription £4.99-19.99/mo. Reddit/TikTok brand-build.
